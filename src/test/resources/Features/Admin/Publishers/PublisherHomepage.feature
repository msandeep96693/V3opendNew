...
Feature: Publisher Home Page
 ...
 Background:
  ...
  @Smoke
  Scenario: The admin filters the publisher by status and verifies the results in the table
   When The admin selects "Active" from the "All" filter dropdown
   Then Verify the table should display only publishers with the status "Active" from "Status" column
